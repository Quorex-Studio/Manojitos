                ...
                {isExpanded && (
                  ...
                  >
                    <ul className="mt-2 space-y-1 bg-background/50 rounded-lg p-2 border border-border/50">
                      {alert.category === 'stock' && Array.isArray(alert.data) && alert.data.map((item: { id: string; name: string; stock: number }) => (
                      ...
                      {alert.category === 'credit' && Array.isArray(alert.data) && alert.data.map((item: { id: string; client_name: string; current_balance: number }) => (
                        <li key={item.id} className="text-xs flex items-center justify-between py-1 px-1 border-b border-border/30 last:border-0">
