...
interface PriceValidityBadgeProps extends HTMLAttributes<HTMLDivElement> {
  showRate?: boolean;
  compact?: boolean;
}
...
export const PriceValidityBadge = forwardRef<HTMLDivElement, PriceValidityBadgeProps>(
  function PriceValidityBadge({ showRate = true, compact = false, ...props }, ref) {
    ...
    if (loading) {
      return (
        <div 
          ref={ref}
          className="flex items-center gap-1.5 text-muted-foreground text-xs animate-pulse"
          {...props}
        >
