...
export function OverdueCreditBanner() {
  const { user } = useAuth();
  const { credit, hasCredit, isLoading } = useCustomerCredit();
  const [dismissed, setDismissed] = useState(false);

  // Solo para usuarios autenticados con crédito vencido/bloqueado
  const isOverdue =
    hasCredit &&
    credit &&
    (credit.calculatedStatus === 'VENCIDO' || credit.is_blocked);
  ...
  return (
    <AnimatePresence>
      ...
      >
        <div className="bg-gradient-to-r from-red-950 via-red-900 to-red-950 border-b-2 border-red-500/60 px-4 py-3">
          <div className="container mx-auto flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
            <div className="flex items-start gap-3 flex-1 min-w-0">
              <div className="p-1.5 rounded-full bg-red-500/20 flex-shrink-0 mt-0.5">
                <AlertTriangle className="h-4 w-4 text-red-400" />
              </div>
              <div className="min-w-0">
                <p className="text-red-100 font-semibold text-sm leading-tight">
                ...
                <p className="text-red-300 text-xs mt-0.5 leading-relaxed">
                  Tienes pagos pendientes. No puedes realizar nuevas compras hasta regularizar tu cuenta.
                </p>
              </div>
            </div>
