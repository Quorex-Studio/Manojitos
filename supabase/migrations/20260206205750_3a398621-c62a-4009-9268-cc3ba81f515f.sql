...
BEGIN
  ...
  -- 4. Loop again to execute changes (Deduct stock & Create Sales)
  FOREACH v_item IN ARRAY items
  LOOP
    ...
    INSERT INTO public.sales (
      ...
      unit_price_usd, total_usd, total_bs,
      payment_method, client_name, client_phone,
      is_credit, notes
    )
    VALUES (
      v_user_id, v_item.id, v_item.name, v_item.quantity,
      v_item.price_usd, v_current_item_total, v_current_item_total_bs,
      payment_method, client_name, client_phone,
      FALSE, notes
    )
